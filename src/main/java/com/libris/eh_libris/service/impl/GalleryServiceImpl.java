...
//                .block();
//
//        System.out.println("EH 返回的原始内容: " + rawResponse);

        // 2. 发起 POST 请求并等待响应 (使用 .block() 简单同步处理)
        EhApiResponse response = ehWebClient.post()
                .uri("/api.php")
                .header("Content-Type", "application/json") // 明确告诉服务器：我发的是 JSON
                .header("Accept", "application/json")       // 明确告诉服务器：我只要 JSON
                .bodyValue(requestBody)
                .retrieve()
                .bodyToMono(EhApiResponse.class)
                .block(); // 在实际高并发场景建议异步，但现在学习阶段 block 最直观
...
//}
    ...
    public List<Gallery> listWithPagination(int pageNum, int pageSize) {
        ...
        Map<Long, List<Long>> galleryTagMap = relations.stream()
                .collect(Collectors.groupingBy(
                        GalleryTagMap::getGid,
                        Collectors.mapping(GalleryTagMap::getGid,Collectors.toList())
                ));

        // 5. 遍历画廊，把对应标签填入
        for (Gallery gallery : galleries) {
            List<Long> tids = galleryTagMap.get(gallery.getGid());
